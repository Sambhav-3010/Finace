...
export default function ReportReviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleReview = async (status: "verified" | "rejected") => {
    ...
    try {
      await reportsApi.submitReview(id, { status, evaluator_remarks: remarks });
      await fetchReport();
      router.refresh();
    } catch (error) {
      console.error(error);
  ...
  const handleGenerateProof = async () => {
    ...
    try {
      ...
      console.error(error);
      alert("Failed to generate proof. Ensure Pinata API and Private Key are configured.");
    } finally {
      setSubmitting(false);
      setActionType(null);
    }
  ...
  if (!report) {
    return (
      <div className="glass rounded-[1.8rem] p-12 text-center border-white/10">
        ...
        <p className="text-white/50 mt-2 mb-6">The requested analysis might have been deleted or doesn't exist.</p>
        <Link href="/dashboard/evaluator" className="bg-accent text-ink px-6 py-2.5 rounded-full font-semibold">
          Back to Reports
        </Link>
      </div>
    );
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h2 className="text-2xl font-semibold text-white flex items-center gap-3">
              Workflow Analysis
              {isFinalized && <Lock className="w-4 h-4 text-emerald-400" title="Anchored on-chain" />}
            </h2>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          <button
            ...
            disabled={submitting}
            className="flex items-center gap-2 rounded-full border border-white/10 px-4 py-2 text-sm text-white/70 hover:bg-white/5 transition disabled:opacity-50"
          >
            {actionType === "updating" ? <Loader2 className="w-4 h-4 animate-spin" /> : <RefreshCw className="w-4 h-4" />}
            Refresh Analysis
          </button>
          ...
          {!isFinalized && report.status === "verified" && (
            ...
            >
              {actionType === "proofing" ? <Loader2 className="w-4 h-4 animate-spin" /> : <Fingerprint className="w-4 h-4" />}
              Generate Proof
            </button>
          )}
        </div>
            ...
            >
              ...
              <ul className="space-y-4">
                {(report.risk_flags || []).map((flag: string, i: number) => (
                  <li key={i} className="flex gap-3 text-sm text-white/70 leading-6">
                    <span className="shrink-0 h-1.5 w-1.5 rounded-full bg-rose-400 mt-2.5" />
                    {flag}
                  </li>
                ))}
                {(!report.risk_flags || report.risk_flags.length === 0) && (
                  <p className="text-white/30 text-sm italic">No specific risk flags detected.</p>
                )}
              </ul>
            </motion.div>
            ...
            <div className="space-y-6">
              {(report.applicable_clauses || []).map((clause: any, i: number) => (
                <div key={i} className="border-l-2 border-accent/20 pl-6 py-1">
                  ...
                  <p className="text-sm text-white/50 leading-relaxed italic line-clamp-3 hover:line-clamp-none transition-all cursor-pointer">
                    &quot;{clause.text}&quot;
                  </p>
                </div>
              ))}
              {(!report.applicable_clauses || report.applicable_clauses.length === 0) && (
                <div className="text-center py-10 opacity-30">
                  ...
                  <p>No legal clauses linked to this report.</p>
                </div>
              )}
            </div>
          </motion.div>
        ...
        <div className="space-y-6">
          ...
          <div className="glass rounded-[2rem] p-6 border-white/10">
            ...
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="text-right">
                ...
                <span className={`inline-flex items-center rounded-full px-4 py-1 text-sm font-bold border ${
                  report.risk_level === "HIGH" ? "bg-rose-500/10 text-rose-400 border-rose-500/20" :
                  report.risk_level === "MEDIUM" ? "bg-amber-500/10 text-amber-400 border-amber-500/20" :
            ...
            <div className="pt-6 border-t border-white/5 space-y-4">
              ...
              <div className="flex justify-between text-sm">
                <span className="text-white/40">Regulator</span>
                <span className="text-white/80">{report.workflow_input?.regulator || "RBI"}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-white/40">Created At</span>
                <span className="text-white/80">{new Date(report.created_at).toLocaleDateString()}</span>
              </div>
            </div>
          </div>
          ...
          {report.status !== "verified" && (
            <div className="glass rounded-[2rem] p-6 border-white/10">
              ...
              <textarea
                className="w-full bg-white/[0.03] border border-white/10 rounded-2xl p-4 text-sm text-white focus:outline-none focus:ring-1 focus:ring-accent/40 transition mb-4 min-h-[120px] placeholder:text-white/20"
              ...
              <div className="flex gap-3">
                <button
                  ...
                  disabled={submitting}
                  className="flex-1 rounded-2xl bg-emerald-500 text-ink py-3 text-sm font-bold transition hover:bg-white disabled:opacity-50"
                >
                  {actionType === "verified" ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : "Verify Analysis"}
                </button>
                <button
          ...
          {isFinalized && (
            ...
            >
              ...
              <div className="space-y-4">
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-white/30 mb-1.5">IPFS Storage (CID)</p>
                ...
                <div className="pt-2 text-center">
                  <p className="text-[10px] text-white/30 italic">Compliance proof immutable and verified by Base Sepolia validators.</p>
                </div>
              </div>
            </motion.div>
          )}

          {/* Remarks History if rejected/verified */}
